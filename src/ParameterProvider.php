...
class ParameterProvider {
	...
	public function getEscapedUploadPageText() {
		...
		return '{{' . $this->getEscapedParameter( self::IDX_TEMPLATENAME ) .
			   $this->getEscapedParameter( self::IDX_TEMPLATEPARAMETERS ) . '}}';
	}
	...
	private function populateParametersFromKey() {
		...
		if ( $paramMsg->exists() ) {
			...
			if ( array_key_exists( $this->templateName, $paramSet ) ) {
				$this->setParameters( $this->templateName, '', $paramSet[$this->templateName][1],
					$paramSet[$this->templateName][2] );
				return true;
			}
		}
		return false;
	...
	private function populateParametersFromDefaults() {
		$this->setParameters( $this->templateName, '', Message::newFromKey( 'simplebatchupload-comment' )
															  ->text(), Message::newFromKey( 'batchupload' )->text() );
	}
